...
uploading or registering a DuckDB virtual file. Each run keeps its URLs
alive through all queries and revokes them on success or failure.
Selected files remain available for reruns; **Clear local files** or
choosing a bundled dataset releases the selection. The original
annotation filename identifies GTF even though the URL has no suffix;
headers and gzip decoding go through DuckHTS.
