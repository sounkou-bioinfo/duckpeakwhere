...
Run **Where** on a dataset, then open the **SQL console** below the
results. Paste either query into the console and click **Run**:

``` sql
SELECT count(*) AS transcripts FROM tx;
```

``` sql
SELECT fid, count(*) AS peaks FROM peak WHERE reason IS NULL GROUP BY fid ORDER BY fid;
```

The first reads Where’s transcript table; the second counts accepted
peaks by file ID. **Show the SQL** beneath each view’s results lists the
statements used for that run.
